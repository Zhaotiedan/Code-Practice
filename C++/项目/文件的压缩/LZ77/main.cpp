...
2.哈希
   哈希表：head+prev 一共64k。head：字符串通过哈希函数计算出来的哈希地址作为下标，保存首字符在查找缓冲区中的下标。prev用来解决哈希冲突
   哈希表大小：取决于哈希函数计算出来的哈希地址--哈希表需要保存所有的哈希地址。哈希函数计算：将三个字符的比特位进行操作
              由于三个字符的排列组合一共有2^8 * 2^8 * 2^8=2^24种可能，所以计算出来的哈希地址就是2^24中可能，所以哈希表需要空间个数为2^24
	      又因为一个空间里的下标范围是2个字节  所以理论上哈希表需要空间个数=  2^24*2=32M
	      但是，LZ77最终给的哈希表空间个数为2^15
   哈希表的放入字符过程：
                pos代表该组字符(三个字符)首字符在先行缓冲区中的位置；
